...
exports.redactUnrevealedCards = redactUnrevealedCards;
function escapeRegExp(input) {
    return input.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function baseTitle(cardTitle) {
    return cardTitle.replace(/,\s*reversed$/i, '').trim();
}
function buildCardRegex(cardTitle) {
    const title = escapeRegExp(baseTitle(cardTitle));
    // Match upright or ", reversed"
    return new RegExp(`\\b${title}(?:,\\s*reversed)?\\b`, 'gi');
}
function redactUnrevealedCards(chapterText, spreadCards, revealedCards, clarifierCards) {
    ...
    const clarifierRevealLine = new RegExp(`^\\s*Clarifiers?\\s*:\\s*(.+)$`, 'i');
    const clarifierWordRegex = /\b[Cc]larifiers?\b:?/g;
    const redactClarifierLine = (line) => {
        let redacted = line;
    ...
    for (const { pattern, replacement } of energyReplacements) {
        if (energyCount <= 10)
            break; // Stop if already below target
    ...
    console.log(`[validator] Energy pass: initial=${initialEnergyCount}, patternMatches=${totalEnergyPatternMatches}, remaining=${energyCount}`);
    const signaturePatternsForLog = [
        /\bYou knew[^.!?\n]*(?:before you said it|this already|that already)/gi,
        /\bDon't lie to yourself[^.!?\n]*/gi,
    ];
    let signatureMatchesForLog = 0;
    for (const pattern of signaturePatternsForLog) {
        const matches = finalText.match(pattern);
        if (matches) {
            signatureMatchesForLog += matches.length;
        }
    }
    console.log(`[validator] Signature pass (baseline): matched=${signatureMatchesForLog}, softened=0`);
